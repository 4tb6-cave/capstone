...
\title{\textbf{Revision 1: \\ System Design \\ \large Group 8 - C.A.V.E: Cave Assessment and Visualization Equipment}}
...
\begin{document}
	...
	\begin{table}[hp]
		...
		\begin{tabularx}{\textwidth}{llX}
			...
			\midrule
			December 22, 2025 & All Members & Create document for Revision 0 design\\
            \midrule
            March 16, 2026 & All Members & Added module M5 and updated uses hierarchy \newline 
                Added section 1.4 Design Overview \newline
                Provided additional detail in Module Information, including S1-S2 interface and requirements \\
			\midrule
            March 18, 2026 & Berk Yilmaz & Updated ToF sensor interface from USB to UART \\
            \midrule
            March 18, 2026 & Andrew Brink & Remove 2D LiDAR from system design\\
			\bottomrule
		\end{tabularx}
	\end{table}
    ~\newpage
	\section{Introduction}
        ...
        \subsection{Design Overview}
            Two main systems make up the device, with a portable Raspberry Pi connecting to sensors, storing their captured data, and providing interfaces for the user to interact with. This section of the device is referred to as the Data Collection subsystem (S1). After the caving expedition is complete, the Point Cloud Processing subsystem (S2) uses ICP and GTSAM to process the captured point clouds and IMU data to produce 3-D maps of the capture area. Researchers and other cavers can then view this 3-D object to see a representation of the cave. The subsystem division chosen is natural due to the different timing requirements of the tasks they complete. S1 is lightweight and focuses on rapidly capturing the data, S2 occurs after measurement, likely after returning to base from the expedition, so it can be done on more powerful hardware at a slower pace with human intervention if necessary.
...
\begin{figure}[H]
				...
				\caption{Dimensions of Raspberry Pi 5 \cite{rpi_mechanical}}
				\label{fig:raspi5_D}
			\end{figure}
		\subsubsection{Electrical Interfaces}
			Refer to the Raspberry Pi RP1 Peripherals reference \cite{rpi_peripherals}. 

	\subsection{ToF Sensor}
			The time-of-flight sensors used are SIPEED MaixSense A010 3D ToF sensors. Information is taken from \cite{tof}.
		\subsubsection{Inputs and Outputs}
			\begin{tabularx}{\textwidth}{X X X p{0.45\textwidth}}
				\toprule
				\textbf{Direction} & \textbf{Name} & \textbf{Format} & \textbf{Description} \\
				...
				Output & ToF Data & UART & 3D point cloud, continous scans (M\_tof\_data) \\
				\bottomrule
			\end{tabularx}
		\subsubsection{Behavior}
...
\begin{figure}[H]
			...
			\end{figure}
	\subsection{IMU}
		This component is the Adafruit BNO055 Absolute Orientation Sensor. Further information is found in \cite{imu}. 
		\subsubsection{Inputs and Outputs}
			\begin{tabularx}{\textwidth}{X X X p{0.45\textwidth}}
				\toprule
				\textbf{Direction} & \textbf{Name} & \textbf{Format} & \textbf{Description} \\
...
% 			Four pin JST-GH cable carrying 5V, UART Rx, UART Tx, and ground. 
	...
	\subsection{Battery Pack}
		The battery pack being used is the UGREEN Nexode 100W 3-Port 20,000mAh power bank \cite{power_bank} as it provides full 20V PD at a reasonable price and size. 
		\subsubsection{Inputs and Outputs}
			\begin{tabularx}{\textwidth}{X X X p{0.45\textwidth}}
				\toprule
				\textbf{Direction} & \textbf{Name} & \textbf{Format} & \textbf{Description} \\
	...
	\subsection{Helmet Mount}
		\subsubsection{Inputs and Outputs}
			This module has no electrical signals or information to process and so has no inputs or outputs. From a mechanical standpoint its inputs are the masses of the TOF sensors, IMU, and Raspberry Pi, and its output is their combined masses loaded on the helmet. 
		\subsubsection{Behavior}
			The purpose of this module is to provide a stable surface for the helmet mounted components to be secured to a caving helmet. The ToF module must be in a position where it can carry out uninterrupted scans, facing directly forward to capture what the user is looking at. To enable the custom shapes needed to hold various sizes of sensors on a round platform the material will likely be 3-D printed PLA plastic. This material provides sufficient strength for the relatively small and light sensors and allows heat-set inserts to securely screw the each component to it. The primary requirement to conform to is RE2.
		\subsubsection{Dimensions}
	...
	\section{Software Subsystem S1: Data Collection}
		...
		\label{sec:M1}
			\subsubsection{Module Information}
				The ToF module is responsible for acquiring depth measurement data from the sensors and outputting time-stamped point cloud data. The module provides raw 3D point cloud frames in continuity that represent the surrounding environment. The hidden design decision in this module is the calibration parameters (i.e. serial access between sensor and the local system) and the calculation process from depth values to 3D point cloud maps. Achieves requirements RB1, RB5, RT1, RT2, RE2.
			\subsubsection{Uses Hierarchy}
			Uses: TOF Sensor (sect 2.2) \\
		...
		\label{sec:M2}
			\subsubsection{Module Information}
			The IMU provides inertial data to the Raspberry Pi, which will be used to improve the accuracy of our final model. This module reports the real-time rotation and acceleration data, which is logged for later use. Both the raw values of the 3D accelerometer/gyroscope and the on-chip fusion provided by the sensor are captured so that either may be used during point-cloud processing if results need to be tuned. Achieves requirements RB1, RT1, RT2, RE2.
			\subsubsection{Uses Hierarchy}
				Uses: IMU (sect. 2.3) \\
			...
			\subsubsection{MID}
				...
				\begin{itemize}
					\item sensor\_acceleration
						\begin{itemize}
							\item Description: A getter function to read acceleration data from the BNO055 sensor over I2C 
							\item Parameters: None 
							\item Returns: Acceleration data
						\end{itemize}
					\item sensor\_gyro
						\begin{itemize}
							\item Description: A getter function to read gyroscopic data from the BNO055 sensor over I2C 
							\item Parameters: None 
							\item Returns: Gyroscope data
						\end{itemize}
					\item sensor\_linear\_acceleration
						\begin{itemize}
							\item Description: A getter function to read linear acceleration data from the BNO055 sensor over I2C 
							\item Parameters: None 
		...
		\label{sec:M3}
			\subsubsection{Module Information}
			The data aggregation module is responsible for producing time-aligned sensor data and synchronize each ToF point cloud frame with the IMU data to the closest time ($\Delta_{t\_IMU} - \Delta_{t\_point\_cloud}$) it was acquired. This ensures that data processing is made as easy as possible by reducing the phase delay between measurements used in the GTSAM algorithm (M7). Achieves requirement RT1.
			\subsubsection{Uses Hierarchy}
				Uses: Module M1 (ToF sensor) and Module M2 (IMU)\\ 
				Used by: Module M4 (Rosbag Recorder)
			\subsubsection{MIS}
				\begin{itemize}
					\item Inputs:
						\begin{itemize}
		...
		\subsection{Module M4: Rosbag Recorder}
			\subsubsection{Module Information}
				This module records the synchronized sensor data packet and stores it as a rosbag file so that it can be used later in the point cloud processing. Rosbags are a well-supported datastructure that enable the large swath of captured sensor data to be packaged together in a logical manner and later unpacked by M6. Achieves requirement RE1.
			\subsubsection{Uses Hierarchy}
				Uses: Module M3 (Data Aggregation) \\
				Used By: Module M5 Recording Manager, Module M6 (Data Ingest)
			\subsubsection{MIS}
				\begin{itemize}
					\item Input:
						\begin{itemize}
			...
			\end{itemize}
			\subsubsection{Scheduling}
				Started by M5 according to M\_start\_stop
        \subsection{Module M5: Recording Manager}
			\subsubsection{Module Information}
				This module runs continuously to respond to user input through M\_start\_stop and also enables the system to be shut down gracefully and the captured information saved. It also checks system storage and alerts the user if recording space is running out. Remaining recording time is not handled by the system; the user can evaluate how long to continue recording based on a combination of the storage full LED on the Raspberry Pi and the battery percentage indicator on the USB-C battery pack. See Appendix A (Sandmaze PCBs) for further information on the physical button and LED layout. Achieves requirements RB3, RB4, RT2, RE2.
			\subsubsection{Uses Hierarchy}
				Uses: Module M3 (Data Aggregation), Module M4 (Rosbag Recorder) \\
				Used By: Module M6 (Data Ingest)
			\subsubsection{MIS}
				\begin{itemize}
					\item Input:
						\begin{itemize}
							\item Start button (M\_start\_stop)
							\item Power off button
						\end{itemize}
					\item Output:
						\begin{itemize}
							\item Recording status (C\_record\_status)
                            \item Storage fullness (Remaining time) (C\_record\_remain)
						\end{itemize}
				\end{itemize}
			\subsubsection{MID}
			Methods:
			\begin{itemize}
				\item shutdown\_isr
					\begin{itemize}
						\item Description: Enables interrupts for the shutdown button
						\item Parameters: User actuation of Button 2
						\item Returns: Sets local state variable C\_SHUTDOWN to TRUE
					\end{itemize}
				\item rec\_toggle\_isr
					\begin{itemize}
						\item Description: Enables interrupts for the recording toggle button
						\item Parameters: User actuation of Button 1
						\item Returns: Toggles the boolean value of local state variable C\_START\_STOP
					\end{itemize}
                \item check\_capacity
					\begin{itemize}
						\item Description: Calculates storage usage on the device
						\item Parameters: None
						\item Returns: TRUE if storage usage exceeds the threshold 80\%, FALSE otherwise
					\end{itemize}
				\item print\_subprocess\_result
					\begin{itemize}
						\item Description: Routes Docker log outputs to appropriate streams
						\item Parameters: 'result' struct from subprocess.run()
						\item Returns: Text output on stdout or stderr as appropriate
					\end{itemize}
                \item stream\_docker\_logs
					\begin{itemize}
						\item Description: Polls detached Docker process implementing M5 for log information
						\item Parameters: None
						\item Returns: Prints new lines of log output (as determined by a small local cache) to stdout
					\end{itemize}
			\end{itemize}
			\subsubsection{Scheduling}
				Starts upon Pi power-up, ends when S1 is shut down through shutdown\_isr
	\section{Software Subsystem S2: Point Cloud Processing}

		\subsection{Module M6: Data Ingest}
		\label{sec:M4}
			\subsubsection{Module Information}
				The Data Ingest module is part of the Point Cloud Processing subsystem. The responsibility of this module is to take the data previously collected by the Data Collection subsystem, which is stored in the form of a rosbag, and prepare it for use by the other modules in this subsystem. The rosbag captured by the mobile device implementing S1 is moved to a personal computer for processing by S2 manually, either over a network (Secure Copy Protocol) or by removing the SD card in the Raspberry Pi and mounting locally. Achieves requirement RE1 (All behavioural requirements were achieved in S1).
			\subsubsection{Uses Hierarchy}
				Uses: Module M4 (Rosbag Recorder)\\
				Used by: Module M7 (Sensor Fusion) 
			\subsubsection{MIS}
				\begin{itemize}
					\item Input:
						\begin{itemize}
...
\end{itemize}
		...
		\subsection{Module M7: Sensor Fusion}
			\subsubsection{Module Information}
			This module uses the data from the IMU and ToF sensors to build a 3D point cloud. The IMU data is integrated to provide an initial estimate of the transformation between two ToF captures. This estimate is used to perform point cloud alignment for each new point cloud produced by the ToF cameras. GTSAM is used to integrate the IMU information with the point clouds and improvement pose estimation. Manual intervention is used to identify start and end frames to allow for loop closure, and additional intervention may be done to help 'steer' the ICP algorithm towards more correct results. Achieves requirement RB2.
			\subsubsection{Uses Hierarchy}
				Uses: Module M6 (Data Ingest)\\
				Used by: None 
			\subsubsection{MIS}    
			\begin{itemize}
		\item Inputs 
...
\textit{The work of Chen et al. \cite{chen} has been used as a reference for how the Error State Kalman filter and ToF ICP can be implemented, and their open-source code will be used as a reference as well. This work is limited odometry, however, meaning it does not include any loop closure. }
%\subsubsection{Initialization}

\subsubsection{Scheduling}
There are no scheduling requirements, as this subsystem does not operate in real time.  
	...
	\section{Appendix A: Sandmaze PCBs}
		...
		\subsection{Rpi Head} 
			...
			\begin{tabularx}{\textwidth}{X X X p{0.45\textwidth}}
				...
				Input & ToF Data & UART & Series of point maps as captured by the time-of-flight sensor (M\_tof\_data) \\
...
\newpage
		\begin{thebibliography}{7}
			\bibitem{rpi_mechanical} Raspberry Pi 5 Mechanical Drawing, \url{https://datasheets.raspberrypi.com/rpi5/raspberry-pi-5-mechanical-drawing.pdf} (accessed Dec. 22, 2025).
			\bibitem{rpi_peripherals} Raspberry Pi RP1 Peripherals, \url{https://datasheets.raspberrypi.com/rp1/rp1-peripherals.pdf} (accessed Dec. 22, 2025).
			\bibitem{tof} Sipeed MaixSense A010 Documentation, \url{https://dl.sipeed.com/shareURL/MaixSense/MaixSense_A010} (accessed Dec. 22, 2025).
			\bibitem{imu} BNO055 IMU Datasheet, \url{https://learn.adafruit.com/adafruit-bno055-absolute-orientation-sensor/downloads} (accessed Dec. 22, 2025).
			\bibitem{power_bank} UGREEN Nexode 100W 3-Port 20,000 mAh Power Bank product page, \url{https://www.staples.ca/products/3119320-en-ugreen-nexode-100w-3-port-20000-mah-power-bank-portable-charger-silver} (accessed Dec. 22, 2025).
			\bibitem{chen} S. Chen, C.-W. Chang, and C.-Y. Wen, “Perception in the dark; development of a TOF visual inertial odometry system,” Sensors, vol. 20, no. 5, p. 1263, Feb. 2020. doi:10.3390/s20051263 
		\end{thebibliography}
