...
if [ "$#" -ne 5 ]; then
    echo "Usage: $0 <source_dir> <destination_dir> <start_time> <end_time> <topic_name>"
    exit 1
fi
...
shopt -s nullglob
files=("$source_dir"/"$topic_name"_*.png)
if [ ${#files[@]} -eq 0 ]; then
...
for ((i = 0; i < total_files; i++)); do
    ...
    timestamp=$(echo "$filename" | grep -oP '(?<='$topic_name'_)\d+\.\d+(?=\.png)')
